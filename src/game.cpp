...
Game::Game(Point draw_pos, Point size)
    : snake(Point{0, 0}, Heading::RIGHT, 0), draw_pos(draw_pos), size(size),
      score(0) {
...
void Game::reset() {
  game_over = false;
  score = 0;
  snake.init(Point{field_size.x / 2 - SNAKE_START_SIZE, field_size.y / 2},
             Heading::RIGHT, SNAKE_START_SIZE);
  move_fruit();
}
...
void Game::move_fruit() {
  ...
  do {
    fruit.x =
        rand() % (field_size.x -
                  1); // -1 because fruits are emojis and therefore wide chars
...
void Game::box(WINDOW *win, Point position, Point size) {
  for (int x = 1; x <= size.x - 2; x++) {
    ...
    mvwprintw(win, position.y + size.y - 1, position.x + x, "%s",
              HORIZONTAL_LINE);
  }
  for (int y = 1; y <= size.y - 2; y++) {
    mvwprintw(win, position.y + y, position.x, "%s", VERTICAL_LINE);
    mvwprintw(win, position.y + y, position.x + size.x - 1, "%s",
              VERTICAL_LINE);
  }
  // Corners
  ...
  mvwprintw(win, position.y + size.y - 1, position.x + size.x - 1, "%s",
            CORNER_BOTTOMRIGHT);
}
...
void Game::draw_fruit(WINDOW *win) {
  mvwprintw(win, fruit.y + inner_draw_pos.y, fruit.x + inner_draw_pos.x, "%s",
            fruit_str.c_str());
}

void Game::draw_score(WINDOW *win) {
  mvwprintw(win, draw_pos.y + SCORE_POSTION.y, draw_pos.x + SCORE_POSTION.x,
            " Score: %ld ", score);
}

void Game::draw_game_over(WINDOW *win) {
  Point text_pos{draw_pos.x + (size.x - (int)GAME_OVER.size()) / 2,
                 draw_pos.y + size.y / 2};
