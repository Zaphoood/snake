...
Point Snake::get_head() { return head; }
...
bool Snake::check_collision(Point boundaries) {
  if (head.x < 0 || head.x >= boundaries.x || head.y < 0 ||
      head.y >= boundaries.y) {
    return true;
  }
  for (std::deque<Point>::iterator it = ++body.begin(); it != body.end();
       ++it) {
...
Heading Snake::get_heading() { return heading; }
...
void Snake::draw(WINDOW *win, Point offset, int subframe) {
  ...
  if (old_tail.x > 0) {
    ...
    if (subframe == 0) {
      ...
      draw_fractional_block(win, offset + old_tail, tail_heading,
                            N_BLOCKS - subframe);
    }
  }
}

void Snake::draw_fractional_block(WINDOW *win, Point position,
                                  Heading direction, int frac) {
